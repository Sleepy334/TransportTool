...
namespace PublicTransportInfo
{
    internal static class PatchUtil
    {
        ...
        private const string HarmonyId = "TransportTool";
            ...
            MethodDefinition? transpiler = null)
        {
            ...
            {
                throw new Exception(
                    $"TransportTool: prefix, postfix and transpiler are null for method {original.Type.FullName}.{original.MethodName}");
            }

            try
            {
                Debug.Log($"TransportTool: Patching method {original.Type.FullName}.{original.MethodName}");
            ...
            {
                Debug.LogError($"TransportTool: Failed to patch method {original.Type.FullName}.{original.MethodName}");
                Debug.Log(e);
            }
        }
        ...
        {
            Debug.Log($"TransportTool: Unpatching method {original.Type.FullName}.{original.MethodName}");
        ...
        private static MethodInfo GetOriginal(MethodDefinition original)
        {
            ...
            {
                throw new Exception(
                    $"TransportTool: Failed to find original method {original.Type.FullName}.{original.MethodName}");
            }
        ...
        private static MethodInfo GetPatch(MethodDefinition patch)
        {
            ...
            if (methodInfo == null)
            {
                throw new Exception($"TransportTool: Failed to find patch method {patch.Type.FullName}.{patch.MethodName}");
            }
