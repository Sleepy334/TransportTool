...
namespace SleepyCommon
{
    public class NewListView : UIPanel
    {
        ...
        public NewListView() : base() {
            ...
            m_rows = new List<NewListViewRow>();
        }

        public static NewListView? Create(UIComponent oParent, string txtBackgroundSprite, float fTextScale, float fWidth, float fHeight)
        {
            try
            {
                NewListView listView = oParent.AddUIComponent<NewListView>();
                if (listView != null)
                {
        ...
        public void Setup()
        {
            m_header = NewListViewHeader.Create(this, width, null);
            ...
            m_listPanel = m_HorizontalPanel.AddUIComponent<NewListViewMainPanel>();
            if (m_listPanel != null)
            {
        ...
        public UIScrollbar? SetUpScrollbar()
        {
            if (m_scrollbarPanel != null) 
            {
                ...
                m_scrollbar.height = m_scrollbarPanel.height;// TransferBuildingPanel.iLISTVIEW_OFFERS_HEIGHT - m_header.height; // Seem to have to hard code this, not sure why yet...
        ...
        public void AddColumn(NewListViewRowComparer.Columns eColumn, string sText, string sTooltip, int iWidth, int iHeight, UIHorizontalAlignment oTextAlignment, UIAlignAnchor oAncor, NewListViewHeaderColumnBase.OnListViewColumnClick eventCallback)
        {
            if (m_header != null)
            {
        ...
        public NewListViewRow? AddItem(ListData data)
        {
            m_rows.Add(NewListViewRow.Create(m_listPanel, m_backgroundSprite, m_fTextScale, data));
        ...
        public void Clear()
        {
            if (m_rows != null)
            {
                foreach (NewListViewRow row in m_rows)
                {
        ...
        { 
            ...
            if (m_scrollbar != null)
            {
                ...
                Debug.Log("m_scrollbar.height" + m_scrollbar.height + " m_scrollbarPanel.height:" + m_scrollbarPanel.height);
            }
        }

        public void UpdateData(NewListViewRowComparer.Columns eSortColumn, bool bDesc)
        {
            if (m_listPanel != null)
            {
                foreach (NewListViewRow oRow in m_listPanel.components)
                {
                    oRow.UpdateData();
                }
            }
        ...
        public void Sort(NewListViewRowComparer.Columns eSortColumn, bool bDesc)
        {
            if (m_listPanel != null)
            {
